...
export default function BeritaTerbaruDefaultPage() {
  ...
  useEffect(() => {
    axios
      .get(process.env.REACT_APP_ENP_BE + "api/broadcast/allData")
      .then((res) => {
  ...
  return (
    <Fragment>
      ...
      {/* end navbar */}
      <p className="text-center my-5 text-2xl font-bold text-blue-800 underline">
        STIKOMCKID NEWSS
      </p>
        ...
        {allBroadcast.map((item: any, idx: number) => (
          <div>
            <div className="bg-white h-max shadow hover:shadow-2xl hover:cursor-pointer">
              <p className="text-lg p-2 text-center font-semibold mb-5">
                {item.title}
              </p>
              <img className="mx-auto" src={item.imageUrl} />
              <div className="p-5">
                <p className="mt-5 mb-2 text-xs">
                  {moment(item.time_post).format(TIMESTAMP_FORMAT_SHOW)}
                </p>

...
import { N_AddAdmin, N_DeleteAdmin, N_EditAdmin, N_GetListAdmin } from "src/network/admin/admin";
  ...
  return (
    <Fragment>
      ...
      <Snackbar
        ...
        key={'top' + 'center'}
      >
         <Alert onClose={HandleCloseAlert} severity={alert.status === 'success' ? 'success' : 'error'} sx={{ width: '100%' }}>
          {alert.msg}
        </Alert>
      </Snackbar>

      <Drawer anchor="right" open={form.open} onClose={HandleCloseDrawer}>
        <FormActionAdmin setAlert={setAlert} onClose={HandleCloseDrawer} mutate={mutate} type={form.type} oldData={form.oldData} />
      </Drawer>
              ...
              <TableBody>
                {listAdmin?.data?.data?.map((item: any, idx: number) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <ModeEditIcon className='cursor-pointer' sx={{ width: 40, height:40 }} color='info' onClick={() => HandleEdit(item)} />
                      <DeleteIcon className='cursor-pointer' sx={{ width: 40, height:40 }} color='error' onClick={() => HandleDelete(item)} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
...
type IFormActionAdmin = { onClose: any, mutate: any, setAlert: any, oldData: any, type: any };
const FormActionAdmin = (props: IFormActionAdmin) => {
    ...
    if (type === 'edit'){
      await N_EditAdmin(form, oldData._id)
          ...
          .catch(err => {
            ...
            setAlert({ open: true, msg: err.response.data.msg, status: 'failed' })
          })
  ...
  return (
    <div className="w-96 p-5 h-full">
      ...
      <h3 className="font-semibold text-lg">{type === 'add' ? 'Add New' : 'Edit' } Admin</h3>
      <Divider />

      <form className="py-5 h-full flex flex-col justify-between" onSubmit={Submit}>
        <div className="space-y-2">
          ...
          <TextField
            ...
            variant="filled"
            value={form.phoneNumber.includes('+62') ? parseInt(form.phoneNumber.replace('+62','')) : parseInt(form.phoneNumber)}
            fullWidth
            name='phoneNumber'
            type='number'
            onChange={(e: any) => setForm({ ...form, phoneNumber: `+62${e.target.value}` })}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <LocalPhoneIcon /> +62
          ...
          <div className="flex items-center justify-end space-x-2">
            ...
            <LoadingButton variant='contained' type='submit' loading={loadingFetch}>Submit</LoadingButton>
          </div>

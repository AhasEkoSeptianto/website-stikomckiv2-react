...
class Settings extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      ...
      // </div>
      <div className="grid lg:grid-cols-12 lg:gap-10 space-y-4 lg:space-y-0">
        <div className="col-span-12 lg:col-span-3">
          <div className="bg-white shadow rounded">
            <p className="p-3 text-lg font-semibold bg-gray-700 text-white">
              Settings
            </p>
            <Divider />
            <div>
              {listSetting.map((item, idx) => (
                <Link to={`/dashboard/settings?tab=${item.params}`}>
                  <p
                    className={`px-5 py-2 hover:bg-gray-300 cursor-pointer font-normal ${item.params === tab ? "bg-gray-300" : ""}`}
                    key={idx}
                  >
                    {item.label}
                  </p>
